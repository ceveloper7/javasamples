...
import org.ceva.ch07.util.EnumCombo;
import org.ceva.ch07.util.GBC;
 ...
 * display formatted date and time, and buttons to parse the text field contents.
 */
class DateTimeFormatterFrame extends JFrame
{
    ...
    public DateTimeFormatterFrame()
    {
        ...
        {
            currentDate = LocalDate.parse(dateText.getText().strip(), currentDateFormat);
        });
        addAction(timeParseButton, () ->
        {
            currentTime = LocalTime.parse(timeText.getText().strip(), currentTimeFormat);
        });
        addAction(dateTimeParseButton, () ->
        {
            currentDateTime = ZonedDateTime.parse(
                    dateTimeText.getText().strip(), currentDateTimeFormat);
        });
